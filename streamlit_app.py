...
def render_theme():
    ...
    .card,.budget-card,.mini-card,.persona-card,.insight-card,.ci-card,.flow-node,.merge-node,.placeholder,.date-card{border-radius:20px;padding:18px;background:#fff}.card{min-height:270px;border-radius:24px}.budget-card{min-height:190px}.date-card{min-height:185px}.mini-card{min-height:175px}.persona-card{min-height:425px}.insight-card{min-height:160px;margin-top:18px}.ci-card{min-height:390px;margin-top:18px}.flow-node{min-height:170px}.merge-node{min-height:190px}.placeholder{min-height:190px;border:1.5px dashed #cbd5e1}
...
def render_budget_plan():
    ...
    st.markdown('<div class="insight-card pastel-mint"><span class="pill bot">AD MANAGEMENT NOTE</span><h4>แผนตอน Manage Ads</h4><p>เริ่มแบ่งงบให้ Traffic CI 2 ตัวใกล้เคียงกันก่อน วัด Persona ที่ให้ signal ดีกว่า แล้วเอาคนดูวิดีโอ / Engage / Click / Comment / Share เข้า Retarget Pool ค่อยโยกงบไป MID Retarget หรือแตก angle เพิ่ม</p></div>', unsafe_allow_html=True); close_module()

def render_date_project():
    open_module('00_DATE_PROJECT_MODULE','วันที่ 1–3 สำหรับ 3 CI หัวข้อหลัก เพื่อคุม production และ media test ให้เป็นรอบ')
    d1,d2,d3=st.columns(3)
    with d1: st.markdown('<div class="date-card pastel-pink"><span class="pill pink">DAY 1</span><div class="date-number">วันที่ 1</div><h4>CI 01 · คู่รัก</h4><p>ทำครีเอทีฟมุมแฟนไม่โอเคกับกลิ่น / ความใกล้ชิดหายไป / แฟนเป็นคนสะกิดให้เริ่มลด</p></div>', unsafe_allow_html=True)
    with d2: st.markdown('<div class="date-card pastel-lav"><span class="pill mid">DAY 2</span><div class="date-number">วันที่ 2</div><h4>CI 02 · ดูแลผู้สูงอายุ</h4><p>ทำครีเอทีฟมุมคนในบ้านเป็นห่วง / กลิ่นและควันกระทบผู้สูงอายุ / ลูกหลานอยากให้เริ่มลด</p></div>', unsafe_allow_html=True)
    with d3: st.markdown('<div class="date-card pastel-mint"><span class="pill bot">DAY 3</span><div class="date-number">วันที่ 3</div><h4>CI 03 · Retarget / Line OA</h4><p>ทำครีเอทีฟกลางน้ำ: สอบถามแฟน / รีวิวจากคนในบ้าน / ดึง Warm audience เข้า Line OA</p></div>', unsafe_allow_html=True)
    st.markdown('<div class="insight-card pastel-sky"><span class="pill top">PROJECT NOTE</span><h4>Logic วันที่ 1–3</h4><p>Day 1–2 ใช้สร้าง Traffic Signal แยก Persona ส่วน Day 3 ใช้ต่อท่อ Retarget เพื่อพาคนที่เริ่มสนใจไปคุยใน Line OA และเตรียมปิดการขายใน BOT Funnel</p></div>', unsafe_allow_html=True); close_module()
...
def render_traffic_to_retarget_pipe():
    ...
    with cen: st.markdown('<div class="merge-node pastel-amber"><span class="pill amber">RETARGET POOL</span><h4>Warm Engagement + Traffic Intent</h4><p>รวมคนจาก CI ทั้งสองตัวที่มีสัญญาณสนใจ</p><ul><li>Video View / Engaged 7–14 วัน</li><li>Click / LPV / Profile Engage 7–14 วัน</li><li>Comment / Save / Share / Inbox 14–30 วัน</li></ul></div>', unsafe_allow_html=True)
    with r: st.markdown('<div class="pipe-h"></div>', unsafe_allow_html=True)
    st.markdown('<div class="pipe-v"></div><div class="pipe-down">↓</div><div class="merge-node pastel-mint"><span class="pill bot">MID RETARGET</span><h4>ส่งต่อเข้า CI กลางน้ำ</h4><p>ยิง MID CI: สอบถามแฟน / รีวิวจากแฟนหรือคนในบ้าน / ดึงเข้า Line OA</p></div></div>', unsafe_allow_html=True); close_module()
...
def render_mid_target():
    ...
    with m2: st.markdown('<div class="mini-card pastel-sky"><span class="pill top">RETARGET SIGNAL</span><h4>สัญญาณที่ใช้ดึงเข้า MID</h4><p>Video View, Engagement, Click, Save, Share, Comment, Inbox หรือคนที่ตอบสนองกับมุม “อยากให้เขาลดเพื่อเรา”</p></div>', unsafe_allow_html=True)
    ...
    with ci1: st.markdown('<div class="ci-card pastel-pink"><span class="pill mid">MID CI 01 · RETARGET</span><h3>สอบถามแฟน: ถ้าเขาลดได้ คุณอยากให้เขาเริ่มไหม?</h3><p><b>Insight:</b> คนรักไม่ได้อยากบังคับให้เลิกทันที แต่อยากให้เขา “เริ่มลด” เพราะกลิ่นบุหรี่ทำให้ความใกล้ชิดแย่ลง</p><div class="ci-meta"><div><b>Creative Format</b><br>POV Couple / Question scene</div><div><b>Hook</b><br>ลองถามแฟนดู…</div><div><b>Retarget Audience</b><br>คนดู TOP video / Engaged / Click</div><div><b>CTA</b><br>ให้แฟนดู / ทัก Line OA</div></div></div>', unsafe_allow_html=True)
...
def render_footer(): st.markdown('<div class="footer-note">Pastel Marketing Board · Date Project · Traffic CI → Retarget Pool → MID Retarget</div>', unsafe_allow_html=True)
def main():
