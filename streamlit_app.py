...
def render_exec_andromeda_gap():
    ...
    st.markdown('<div class="tube-note"><b>Andromeda เป็นตัวเร่ง</b> แต่เชื้อเพลิงคือ Creative + Offer และเข็มทิศคือ Data Signal ถ้าสองส่วนนี้ยังไม่ชัด ระบบอาจเรียนรู้จาก Click / Message มากกว่า Buyer Signal</div>', unsafe_allow_html=True)
    c1,a1,c2,a2,c3,a3,c4=st.columns([1,.08,1,.08,1,.08,1])
    steps=[('ยอดลดหลังใช้ Advantage+','ระบบเริ่มพึ่ง AI Optimization มากขึ้น แต่ Signal หลังบ้านยังไม่พร้อม','grad-red'),('Creative + Offer ต้องตอบโจทย์','Hook, Pain, ราคา และเหตุผลซื้อ ต้องดึงคนที่มี Intent จริงเข้ามา','grad-gold'),('Pixel / Event ต้องส่งสัญญาณ','ระบบต้องรู้ว่าใครเห็นราคา กด LINE รับราคา และซื้อจริง','grad-cyan'),('AI หา Buyer Pattern','เมื่อ Signal ดีขึ้น Retarget / Lookalike / Scale จะเริ่มแม่นขึ้น','grad-green')]
    for col,x in zip([c1,c2,c3,c4],steps):
        with col: st.markdown(f'<div class="flow-step {x[2]}"><h4>{x[0]}</h4><p>{x[1]}</p></div>', unsafe_allow_html=True)
    for a in [a1,a2,a3]:
        with a: st.markdown('<div class="tube-arrow">→</div>', unsafe_allow_html=True)
    st.markdown('<div class="black-line"></div>', unsafe_allow_html=True)
    l,r=st.columns(2)
    with l:
        st.markdown('<div class="mini-compare grad-red"><span class="black-pill">ถ้าระบบยังไม่พร้อม</span><p>Creative + Offer ไม่คม หรือ Data Signal ไม่พอ → AI เรียนรู้จาก Click / Message → Lead ดูถูก แต่ Close ต่ำ → CAC จริงแพง / Scale ไม่เต็มที่</p></div>', unsafe_allow_html=True)
    with r:
        st.markdown('<div class="mini-compare grad-green"><span class="black-pill">ถ้าระบบพร้อม</span><p>Creative + Offer ตอบโจทย์ + Pixel/Event ส่งสัญญาณ → AI เข้าใจ Buyer Pattern → Lead คุณภาพดีขึ้น → Retarget / Scale ทำงานดีขึ้น</p></div>', unsafe_allow_html=True)
    st.markdown('<div class="flow-ribbon"><h4>Reference Example: ยันหี</h4><p>บางสินค้าอย่างยันหีอาจมี Pixel / Event บางส่วนส่งกลับเข้า Meta แม้ข้อมูลอาจไม่ตรง 100% แต่ยังทำให้ระบบมี Signal มากกว่าการพึ่ง Creative + Message เพียงอย่างเดียว</p></div>', unsafe_allow_html=True)
    black_close()
