...
def render_theme():
    ...
    .hero,.card,.mini-card,.persona-card,.insight-card,.ci-card,.flow-node,.merge-node,.budget-card,.placeholder{border:1px solid var(--line);box-shadow:0 12px 36px rgba(31,41,55,.045)}
    ...
    .card,.budget-card,.mini-card,.persona-card,.insight-card,.ci-card,.flow-node,.merge-node,.placeholder{border-radius:20px;padding:18px;background:#fff}.card{min-height:270px;border-radius:24px}.budget-card{min-height:190px}.mini-card{min-height:175px}.persona-card{min-height:405px}.insight-card{min-height:160px;margin-top:18px}.ci-card{min-height:390px;margin-top:18px}.flow-node{min-height:170px}.merge-node{min-height:190px}.placeholder{min-height:190px;border:1.5px dashed #cbd5e1}
...
def render_footer(): st.markdown('<div class="footer-note">Pastel Marketing Board · Traffic CI → Retarget Pool → MID Retarget</div>', unsafe_allow_html=True)
def main():
    render_theme(); render_header(); render_budget_plan(); render_top_ci_workflow(); render_persona_creative(); render_traffic_to_retarget_pipe(); render_mid_target(); render_test_and_scale_placeholder(); render_lower_funnel(); render_footer()
