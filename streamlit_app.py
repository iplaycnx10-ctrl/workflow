...
def render_header(): st.markdown('<div class="hero"><div class="eyebrow">🍬 Project Board · Pastel Minimal</div><h1 class="title">ลูกอมบุหรี่ — Smoking Candy Workflow</h1><div class="subtitle">แยก Modular เป็นขั้นตอน: วางแผนงาน → แนวทางแก้ไข → รายงานผู้บริหาร</div></div>', unsafe_allow_html=True)
...
def render_traffic_to_retarget_pipe():
    ...
    st.markdown('<div class="visual-flow"><div class="merge-node pastel-amber"><span class="pill amber">RETARGET POOL</span><h4>Warm Engagement + Traffic Intent</h4><p>รวมคนจาก CI ทั้งสองตัว + กลุ่มเก่าที่มีอยู่</p><ul><li>Old Warm: Engaged / Inbox / Click / Video View เดิม</li><li>New Warm: Video View / Engaged 7–14 วัน</li><li>Comment / Save / Share / Inbox 14–30 วัน</li></ul></div></div>', unsafe_allow_html=True)
    close_module()
...
def render_solution_andromeda_module():
...
def render_solution_andromeda_tab():
    render_solution_andromeda_module()

def render_solution_tab():
    tab_andromeda = st.tabs(['อันโดเมด้า'])[0]
    with tab_andromeda:
        render_solution_andromeda_tab()

def render_exec_placeholder():
    st.markdown('<div class="exec-black"><div class="black-kicker">EXECUTIVE REPORT</div><div class="system-label">Report Module Placeholder</div><p class="system-sub">ส่วนรายงานผู้บริหารช่วงท้ายจะทำใหม่ภายหลัง</p></div>', unsafe_allow_html=True)
...
def main():
    ...
    tab_plan, tab_solution, tab_exec = st.tabs(['📌 วางแผนงาน', '🛠️ แนวทางแก้ไข', '📊 รายงานผู้บริหาร'])
    with tab_plan: render_planning_tab()
    with tab_solution: render_solution_tab()
    with tab_exec: render_exec_placeholder()
    render_footer()
